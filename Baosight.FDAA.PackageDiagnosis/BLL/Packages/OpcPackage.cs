...
{
    public class OpcPackage : BasePackage, IInstallation, ILicense
    {
        #region Field
        ...
        public ObservableCollection<OPCClientModule> OpcModules { get; set; }
        ...
        {
            get { return Constants.OPC_PACKAGE_NAME; }
        }
        ...
        public override bool IsConfigured
        {
            get
            {
                var signalNumber = OpcModules.Sum(opcModule =>
                    opcModule.AnalogDataTable.Rows.Count + opcModule.DigitalDataTable.Rows.Count);
        ...
        public override List<CodeInfo> Diagnose()
        {
            ...
            //  步骤一：检查.net2.0是否安装
            var doNet20IsInstalled = DiagnosticHelper.CheckStep(codeInfos, CheckDoNetInstallation);

            //  如果服务不存在则中止检查
            PmsServer.CreateInstance().InitPmsService();
            if (PmsServer.CreateInstance().PmsService == null)
            {
                if (installedIsPassed && doNet20IsInstalled)
                {
            ...
            //  步骤一：检查是否授权
            var licensedIsPassed = DiagnosticHelper.CheckStep(codeInfos, CheckLicense);

            //  未授权则中止检查
            if (!(installedIsPassed && doNet20IsInstalled && licensedIsPassed))
            {
                DiagnosticHelper.NotifyStatus(StatusEvent, DiagnosticStatus.NotPassed, out isPassed);
                return codeInfos;
            }
        ...
        {
            var installationCodeInfo = DiagnosticHelper.CheckInstallation(Constants.OPC_DRIVER_NAME,
                Code.OPCInterface_Installed_Info, Code.OPCInterface_NotInstalled_Error);
        ...
        {
            var doNetCodeInfo = MachineHelper.CheckDotNet20IsInstalled() == false
                ? DiagnosticHelper.MakeCodeInfo(Code.OPCInterface_DoNet20NotInstalled_Error)
                : new CodeInfo();
        ...
        {
            var licenseCodeInfo = DiagnosticHelper.CheckLicense(Constants.OPC_DRIVER_NAME,
                Code.OPCInterface_Licensed_Info, Code.OPCInterface_NotLicensed_Error);
        ...
        private CodeInfo CheckDataReceived()
        {
            var receivedCodeInfo =
                DiagnosticHelper.CheckReceivedData(Code.OPCInterface_NotReceivedData_Error, OpcModules);
        ...
        private CodeInfo CheckPartialDataReceived()
        {
            var partialReceivedCodeInfo = DiagnosticHelper.CheckPartialReceivedData(
                Code.OPCInterface_PartialNotReceivedData_Error,
                OpcModules);
